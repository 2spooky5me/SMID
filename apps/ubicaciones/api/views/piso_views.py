...
class PisoViewSet(GeneralModelViewSet):
    """
    ViewSet para la gestión de pisos del CPV.
    """
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            floor_name_data = serializer.initial_data.get("name")
            
            #Verifica si el registro existe
            torre_existente = self.get_queryset().filter(
                name=floor_name_data.title()
            ).first()
            ...
            if torre_existente:
                ...
                return Response(
                    {
                        "error": f"Ya existe un piso con el nombre: '{floor_name_data}'",
                        "detail": torre.data
                    }, 
                    status=status.HTTP_400_BAD_REQUEST
                )
